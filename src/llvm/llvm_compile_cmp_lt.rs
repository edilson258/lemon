...
impl Llvm<'_> {
	pub fn llvm_compile_cmp_lt(&mut self, binary: &ir::BinInstr) {
		...
		if left.is_int_value() && right.is_int_value() {
			...
			let value = self.cmp_int_values(left_int, right_int, IntPredicate::SLT);
		...
		if left.is_float_value() && right.is_float_value() {
			...
			let value = self.cmp_float_values(left_float, right_float, FloatPredicate::OLT);

			let ptr = self.env.get_ptr_value_unwrap(dest);
			return self.store(ptr, value);
		}
		let message = error_codegen!("unsupported 'lt' {} to {}", left.get_type(), right.get_type());
