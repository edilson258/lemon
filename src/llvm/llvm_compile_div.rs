...
impl Llvm<'_> {
	pub fn llvm_compile_div(&mut self, binary: &ir::BinInstr) {
		...
		if left.is_int_value() && right.is_int_value() {
			...
			let value = match self.builder.build_int_signed_div(left_int, right_int, temp) {
				Ok(result) => result,
				Err(_) => error_codegen!("build int div").report(self.loader),
			};
		...
		if left.is_float_value() && right.is_float_value() {
			...
			let value = match self.builder.build_float_div(left_float, right_float, temp) {
				Ok(result) => result,
				Err(_) => error_codegen!("build float div").report(self.loader),
			};
		...
		let message = error_codegen!("unsupported 'div' {} to {}", left.get_type(), right.get_type());
